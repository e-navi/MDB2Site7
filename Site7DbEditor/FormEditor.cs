...
namespace Site7DbEditor {
    ...
    public partial class FormEditor : Form {
        ...
        private void SetupStyles() {
            ...
            if (dgvBatchPreview != null)
                ApplyDgvStyle(dgvBatchPreview);
        ...
        private void WireEvents() {
            ...
            this.dgvIkou.CellValueChanged += (s, e) => {
                if (e.RowIndex < 0 || e.RowIndex >= dgvIkou.Rows.Count)
                    return;
        ...
        private void BindAllData() {
            ...
            if (dgvIkou.Rows.Count > 0)
                SetCurrentRowSafe(dgvIkou, 0);
            dgvIkou_SelectionChanged(this, EventArgs.Empty);

            if (dgvIbutu.Rows.Count > 0)
        ...
        private void UpdateIkouLSelection(IkouLModel? lineOverride = null) {
            try {
                ...
                if (selectedLine != null) {
                    ...
                    if (userLayerNum < 1 || userLayerNum > 16)
                        userLayerNum = 1;
                    ...
                    if (!layerMatched && cmbLineLayer.Items.Count > 0)
                        cmbLineLayer.SelectedIndex = 0;
            ...
            this.BeginInvoke(new Action(() => {
                try {
                    ...
                    if (selectedIbutu != null) {
                        ...
                        if (lIdx >= 0)
                            cmbIbutuLayer.SelectedIndex = lIdx;
                        else
                            cmbIbutuLayer.Text = $"{targetLayer} 遺物{targetLayer}";
        ...
        private void dgvIkou_CellValidating(object? sender, DataGridViewCellValidatingEventArgs e) {
            if (_isLoadingDatabase || _isUpdatingSelection)
                return;
            if (e.RowIndex < 0 || e.RowIndex >= dgvIkou.Rows.Count)
                return;
        ...
        private void dgvIkouL_CellValidating(object? sender, DataGridViewCellValidatingEventArgs e) {
            if (_isLoadingDatabase || _isUpdatingSelection)
                return;
            if (e.RowIndex < 0 || e.RowIndex >= dgvIkouL.Rows.Count)
                return;
        ...
        private void dgvIbutu_CellValidating(object? sender, DataGridViewCellValidatingEventArgs e) {
            if (_isLoadingDatabase || _isUpdatingSelection)
                return;
            if (e.RowIndex < 0 || e.RowIndex >= dgvIbutu.Rows.Count)
                return;
        ...
        private void dgvKikai_CellValidating(object? sender, DataGridViewCellValidatingEventArgs e) {
            if (_isLoadingDatabase || _isUpdatingSelection)
                return;
            if (e.RowIndex < 0 || e.RowIndex >= dgvKikai.Rows.Count)
                return;
        ...
        private void dgvLayer_CellValidating(object? sender, DataGridViewCellValidatingEventArgs e) {
            if (_isLoadingDatabase || _isUpdatingSelection)
                return;
            if (e.RowIndex < 0 || e.RowIndex >= dgvLayer.Rows.Count)
                return;
            ...
            if (!int.TryParse(noStr, out int numVal)) {
                ...
                foreach (var ikou in _db.IkouList) {
                    ...
                    if (string.Equals(p, prefix, StringComparison.OrdinalIgnoreCase)) {
                        if (int.TryParse(n, out int val) && val > maxVal)
                            maxVal = val;
                    }
                }
                numVal = maxVal + 1;
        ...
        private bool IsMapLayerVisible(int layerId) {
            if (_chkMapLayers == null || _chkMapLayers.Length == 0)
                return true;
        ...
        private void PerformMapHitTest(Point clickPos) {
            ...
            if (activeTab == 0 && chkShowIkou.Checked) {
                HitTestIkou(clickPos, ToCanvasPointLocal, thresholdPx);
            } else if (activeTab == 1 && chkShowIbutu.Checked) {
                HitTestIbutu(clickPos, ToCanvasPointLocal, thresholdPx);
            } else if (activeTab == 2 && chkShowKikai.Checked) {
                HitTestKikai(clickPos, ToCanvasPointLocal, thresholdPx);
            }
        }
        ...
        private void InitBatchUpdateControls() {
            ...
            if (cmbBatchTable.Items.Count > 0)
                cmbBatchTable.SelectedIndex = 0;
            if (cmbBatchFilterOp.Items.Count > 0)
                cmbBatchFilterOp.SelectedIndex = 0;
        }
        ...
        private void cmbBatchTable_SelectedIndexChanged(object? sender, EventArgs e) {
            ...
            if (cmbBatchFilterCol.Items.Count > 0)
                cmbBatchFilterCol.SelectedIndex = 0;
            if (cmbBatchUpdateCol.Items.Count > 0)
                cmbBatchUpdateCol.SelectedIndex = 0;
            ...
            try {
                foreach (var obj in matchingItems) {
                    if (obj is IkouLModel line) {
                        ...
                        } else if (updateCol == "CHIKU")
                            ibutu.Chiku = updateVal;
                        else if (updateCol == "SOUI")
                            ibutu.Soui = updateVal;
                        else if (updateCol == "SYUBETU")
                            ibutu.Syubetu = updateVal;
                        else if (updateCol == "DATE")
                            ibutu.Date = updateVal;
                        successCount++;
