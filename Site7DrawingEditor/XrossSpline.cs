...
namespace Site7DrawingEditor
{
    ...
    public class Xross_Spline
    {
        ...
        private List<Point3D> SanitizeInputPoints(List<Point3D> points)
        {
        ...
        public List<Point3D> Calc3DCurvePoints(List<Point3D> points, int divideStep)
        {
            var clean = SanitizeInputPoints(points);
            if (clean.Count < 2) return points ?? new List<Point3D>();
            ...
            for (int i = 0; i <= createCount; i++)
            {
                ...
                // Guard against wild overshooting
                ...
                result.Add(new Point3D(px, py, pz));
            }

            return result;
        }

        public List<Point3D> Calc3DCloseCurvePoints(List<Point3D> points, int divideStep)
        {
            var clean = SanitizeInputPoints(points);
            if (clean.Count < 3) return points ?? new List<Point3D>();

            // Ensure start and end point match for closed curve
            var first = clean[0];
            var last = clean[^1];
            double endDist = Math.Sqrt((first.X - last.X) * (first.X - last.X) + (first.Y - last.Y) * (first.Y - last.Y) + (first.Z - last.Z) * (first.Z - last.Z));
            if (endDist > 0.0001)
            {
                clean.Add(new Point3D(first.X, first.Y, first.Z));
            }
