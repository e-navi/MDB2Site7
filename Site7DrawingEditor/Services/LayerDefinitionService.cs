...
namespace Site7DrawingEditor.Services
{
    ...
    public class LayerDefinitionService
    {
        ...
        public Color GetColor(LayerGroup group, int index, bool isDarkBackground = true)
        {
            ...
            int tableIdx = colorIdx - 1;
            if (tableIdx >= 0 && tableIdx < LayerManager.LayerTableColors.Length)
            {
                return LayerManager.LayerTableColors[tableIdx];
            }
        ...
        public static void DrawPointMark(Graphics g, PointF center, int markType, float radius, Brush fillBrush, Pen borderPen)
        {
            ...
            switch (markType)
            {
                ...
                case 8: // ★ (五芒星)
                    ...
                    double outerR = radius * 1.15;
                    double innerR = radius * 0.48;
                    for (int i = 0; i < 10; i++)
                    {
