...
namespace Site7DrawingEditor.Services
{
    public class DrawingFrameService
    {
        ...
        /// </summary>
        public void DrawRubberBandFrame(Graphics g, CanvasViewController vc, Size canvasSize, double cx, double cy, double angleDeg, Point mouseScreenPos, bool isRotating)
        {
        ...
        /// マップキャンバスに図枠を描画
        /// </summary>
        public void DrawFrame(Graphics g, CanvasViewController vc, Size canvasSize, bool isDarkBackground)
        {
            if (!IsVisible) return;
            if (canvasSize.Width <= 0 || canvasSize.Height <= 0) return;
            ...
            Color outerColor = LayerDefinitionService.Instance.GetColor(LayerGroup.Sakuzu, 1, isDarkBackground);
            Color innerColor = LayerDefinitionService.Instance.GetColor(LayerGroup.Sakuzu, 2, isDarkBackground);
            Color tomboColor = LayerDefinitionService.Instance.GetColor(LayerGroup.Sakuzu, 3, isDarkBackground);
            Color coordColor = LayerDefinitionService.Instance.GetColor(LayerGroup.Sakuzu, 4, isDarkBackground);
        ...
        private PointF[] ToScreenPoints(CanvasViewController vc, (double surveyX, double surveyY)[] pts, Size canvasSize)
        {
        ...
        /// </summary>
        private void DrawTomboAndCoordinates(Graphics g, CanvasViewController vc, Size canvasSize, (double surveyX, double surveyY)[] innerCorners, bool isDarkBackground, Color tomboColor, Color coordColor)
        {
        ...
        /// </summary>
        private void DrawSingleCoordinateLabel(Graphics g, CanvasViewController vc, Size canvasSize, double val, bool isXAxis, double uMidLeft, double uMidRight, double vMidBottom, double vMidTop, double uMinInner, double uMaxInner, double vMinInner, double vMaxInner, Font font, Brush textBrush, Brush bgBrush)
        {
        ...
        /// 方位記号（北矢印）の描画
        /// </summary>
        private void DrawNorthArrow(Graphics g, CanvasViewController vc, Size canvasSize, PointF[] innerScreen, bool isDarkBackground)
        {
            PointF anchor;
            ...
            bool isMonochrome = false)
        {
            ...
            if (db != null)
            {
                ...
                if (showIkou || showHyoukou)
                {
                    ...
                    foreach (var line in db.MasterIkouLList)
                    {
                        ...
                        Color rawColor = colorByIkou
                            ...
                            : LayerDefinitionService.Instance.GetColor(LayerGroup.Ikou, line.Layer, false);
                        ...
                        {
                            PointF labelPt = SurveyToPaperScreen(pts[0].X, pts[0].Y);
                ...
                if (showIbutu)
                {
                    ...
                    {
                        foreach (var ibutu in db.MasterIbutuList)
                        {
                            ...
                            Color ibColor = isMonochrome ? Color.FromArgb(20, 20, 20) : LayerDefinitionService.Instance.GetColor(LayerGroup.Ibutu, ibutu.Layer, false);
                            ...
                            using (var textBrush = new SolidBrush(ibColor))
                            {
                                ...
                                LayerDefinitionService.DrawPointMark(g, sp, layerItem.Mark, ibRadius, ibBrush, penToUse);
                ...
                if (showKikai)
                {
                    ...
                    using (var kikaiFont = new Font("Yu Gothic UI", kFontPx, FontStyle.Bold, GraphicsUnit.Pixel))
                    {
                        foreach (var k in db.MasterKikaiList)
                        {
                            ...
                            Color kikaiColor = isMonochrome ? Color.FromArgb(30, 30, 30) : LayerDefinitionService.Instance.GetColor(LayerGroup.Kikai, kLayer, false);
                            ...
                            using (var textBrush = new SolidBrush(kikaiColor))
                            {
                                ...
                                LayerDefinitionService.DrawPointMark(g, sp, layerItem.Mark, kikaiRadius, kikaiBrush, penToUse);
