...
namespace Site7 {
    ...
    public class KikaiMan {
        //public FormKikai formKikai = new FormKikai();
        ...
        public TINP3 kp;
        public TINP3 bp;    // 後視点  2022.02.17 Add by A.Iimuro for Site連携
        public TINP3 kpA;
        ...
        public double kh3;
        public XYZ kp1, kp2;    //後方交会　交点
        public TINP3[] tkp = new TINP3[3];
        public TINP3[] tkpA = new TINP3[3];    //後方交会３点用
        ...
        public KikaiMan() {
            kmode = KMODE_KB;
            kp = new TINP3();
            bp = new TINP3();//2022/02/26 Iimuro add
        ...
        public XYZ cnvP(double lng, double angh, double angv) {
            ...
            //lv = lv + kh - sh;
            lv = kp.Z + lv + kh - mh;
            //TINP3 p = new TINP3("", x, y, kp.Z + lv);
            x = Math.Round(x, 3);
            y = Math.Round(y, 3);
            lv = Math.Round(lv, 3);
            XYZ p = new XYZ(x, y, lv);
        ...
        public bool isSokutenSelect() {
            if (kmode == KMODE_KB) {    // 器械点＋後視点(既知点)測定
                if (kr[0].p.Name.Equals("")) return false;
                if (kr[1].p.Name.Equals("")) return false;
            }
            if (kmode == KMODE_BI2) {
                if (kr[0].p.Name.Equals("")) return false;
                if (kr[1].p.Name.Equals("")) return false;
            ...
            if (kmode == KMODE_BI3) {
                if (kr[0].p.Name.Equals("")) return false;
                if (kr[1].p.Name.Equals("")) return false;
                if (kr[2].p.Name.Equals("")) return false;
            }
            return true;
        }
        ...
        public bool calc() {
            ...
            if (kmode == KMODE_BI2) {
                //			kp.Name = "器械点";//2022/02/25 Iimuro コメント化
                //kp.Name = pm.KikaiPName;//2022/02/25 Iimuro add
                kp.Name = "K1";
                ...
                bp.set(kr[0].p);//2022/02/25 Iimuro add
                isCalced = true;
                return true;
            }
            if (kmode == KMODE_BI3) {
